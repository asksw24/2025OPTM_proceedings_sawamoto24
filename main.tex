...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
  The widespread contamination of microplastics (MPs) necessitates comprehensive monitoring across various environments. However, standard analytical methods, such as FTIR and Raman spectroscopy, are rely on point-scanning, which is time-consuming and resource-intensive, limiting their utility for high-throughput screening. Here, we propose a rapid multispectral imaging method using polymer auto-fluorescence under UV excitation. Leveraging the distinct fingerprints observed in Excitation-Emission Matrices (EEMs), our approach optimizes spectral acquisition by selecting a minimal subset of effective spectral bands rather than measuring the full sets of spectra in the EEM. Using a Random Forest-based feature selection, we demonstrated that a subset of 5 spectral bands maximizes the classification performance for 9 representative virgin polymers, achieving a weighted F1-score of 0.92. This significant reduction in spectral dimensionality facilitates the design of simplified hardware using commercially available UV-LEDs, offering a practical and accessible solution for high-throughput MP monitoring.
\end{abstract}
...
Excitation-Emission Matrix (EEM) spectroscopy enables the non-destructive characterization of chemical compositions.
When coupled with supervised machine learning algorithms, it facilitates the robust classification of complex environmental samples~\cite{Hall2007-fl}.
Furthermore, our research group has extended this approach to fluorescence imaging to visualize the spatial distribution of quality attributes.
Specifically, we successfully visualized the distribution of freshness indices (such as K-value and IMP content) in frozen fish fillets~\cite{Bui2018-hw} and achieved non-destructive freshness assessment of shrimp~\cite{Rahman2022-zd}.

Specifically, the combination of fluorescence spectroscopy and machine learning has been established as a powerful tool for rapid classification tasks in food quality assessment. Successful applications have been widely reported, ranging from the monitoring of fish and meat products~\cite{Hassoun2019-of} and classification of olive oils~\cite{Dankowska2019-pr} to the automated detection of contamination on meat surfaces using deep learning~\cite{Gorji2022-pb}. Furthermore, it is well documented that selecting optimal spectral bands, rather than utilizing the full spectrum, directly contributes to system cost reduction and processing efficiency~\cite{noauthor_undated-th, Qin2013-xo}. This optimization strategy facilitates the transition from bulky laboratory equipment to practical, portable hardware implementations suitable for on-site inspection~\cite{Beck2015-zd}.

In this study, we propose a multispectral imaging method specialized for polymer identification based on the EEM.
By utilizing inherent polymer auto-fluorescence under UV excitation, we capture essential spectral features as images.
To begin with, toward the future construction of a high-throughput system, we conducted a feasibility study using a benchtop system equipped with a broadband light source.
...
\begin{figure}[ht]
  ...
  \caption{Measured Excitation-Emission Matrices (EEMs) of the nine types of virgin polymer pellets used in this study.
...
The fluorescence emitted from the sample passed through an emission-side bandpass filter to remove scattered excitation light, and a UV-sensitive camera (BU-56DUV, BITRAN CORPORATION) captured the signal.
...
While these are transparent or white under visible light and difficult to distinguish its type, they exhibit characteristic fluorescence intensity differences under UV excitation (Fig~\ref{fig:samples}(b), (c)).
...
\begin{figure}[ht]
  ...
  Note: The sample arrangement differs between visible and fluorescence images. Despite the random arrangement, we clearly observe characteristic fluorescence intensity differences.
  \label{fig:samples}}
\end{figure}
...
The score improvement subsequently plateaued, and, with the accuracy reached a saturation level (approx. 0.99) around $k=11$.
...
\begin{figure}[htbp]
  ...
  \caption{Results of the feature selection.
    ...
    (b) Selection frequency (x-axis) of each spectral band (y-axis) during 10-fold cross-validation.
...
Fig~\ref{fig:classwise_performance} presents the F1-score transition for each of the 9 classes.
...
\begin{figure}[htbp]
  ...
  The black solid line shows the proposed method, the gray dashed line indicates the full-spectrum baseline (98 bands), and the red dashed line marks the optimal cutoff at $k=5$.
...
\begin{figure}[htbp]
  ...
  F1-Score (y-axis) versus the number of features $k$ (x-axis) for each material. The vertical red dashed line indicates the system-wide optimal cutoff ($k=5$), and the gray dashed line represents the full-spectrum baseline.
...
\begin{figure}[htbp]
  ...
  We observe a distinct confusion between HDPE and PET (True PET is misclassified as HDPE in 35\% of cases), highlighting the spectral similarity between these polymers.
...
In this study, we prioritized optimizing the overall average performance for high-throughput screening.
The feature selection analysis, as shown in Fig~\ref{fig:feature_selection_results}(b), revealed that the top 5 spectral bands were selected in more than 7 out of 10 cross-validation folds.
This high selection frequency demonstrates the robustness and stability of these features.
...
This confirms that identifying a compact and stable feature subset is a rational strategy to ensure generalization capability.
...
Analysis of the feature space using t-SNE revealed that while the clusters for PET and HDPE are spatially separated (indicating sufficient spectral distinguishability), a distinct distribution shift occurred between the calibration and validation datasets.
Despite the inherent separability of the features, the Random Forest model misclassified a significant portion of PET validation samples as HDPE.
This suggests that the model's static decision boundaries could not robustly handle the observed distributional discrepancy between the datasets.
Future work will focus on methods to compensate for such dataset shifts to ensure consistent classification.
...
\begin{figure}[ht]
  ...
  This system uses high-power UV-LEDs and an SBC to achieve a standalone, low-cost solution.
...
Through band selection using the step-wise method, we revealed that a weighted F1-score of 0.92 can be obtained across nine representative plastic types with only 5 spectral bands.
